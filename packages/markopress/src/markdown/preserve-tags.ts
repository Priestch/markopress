...
export interface PreserveTagsPluginOptions {
  /** Directory containing Marko component files (default: 'tags/') */
  tagsDir?: string;

  /** Callback when a Marko tag is detected (for validation) */
  onTagDetected?: (tagName: string, lineNumber: number) => void;
}
 ...
 */
function detectMarkoTags(markdown: string): Token[] {
  const tokens: Token[] = [];
  let id = 0;
 ...
 */
export function preserveTagsPlugin(
  md: MarkdownIt,
  options?: PreserveTagsPluginOptions
): void {
